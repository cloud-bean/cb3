    ...
    render() {
        ...
        if (this.props.books) {
            books = this.props.books.map((item, index)=> {
                return (<BookItem hasCheckedButton={this.props.hasCheckedButton} book={item.book} key={index}
                                  checked={item.isSelected} onChange={()=>this.props.onSelect(index)}></BookItem>)
            });
        }
